...
void FuncFParams::genCode()
{
    ...
    for (int i = 0; i < (int)params.size(); i++)
    {
        ...
        Type *int_type = new IntType(32); // 参数存储地址的类型(IntType)
        Type *type = new PointerType(se->getType());
        SymbolEntry *addr_se = new TemporarySymbolEntry(int_type, SymbolTable::getLabel());
        Operand *addr = new Operand(addr_se);
        SymbolTable ::num++; // Label++用于分配新的符号表项
        SymbolEntry *temp_se = new TemporarySymbolEntry(type, SymbolTable::getLabel());
        Operand *temp_addr = new Operand(temp_se); // 参数存储地址
        Instruction *alloca;
        alloca = new AllocaInstruction(temp_addr, se); // allocate space for local id in function stack.
        entry->insertBack(alloca);                     // allocate instructions should be inserted into the begin of the entry block.
        StoreInstruction *store = new StoreInstruction(temp_addr, addr);
        entry->insertBack(store);
        se->setAddr(temp_addr);
...
void FunctionCall::genCode()
{
    ...
    if (params != nullptr)
    {
        for (int i = 0; i < (int)params->expr.size(); i++)
        {
    ...
    Type *int_type = new IntType(32);
    SymbolTable ::num++;
    SymbolEntry *temp_se = new TemporarySymbolEntry(int_type, SymbolTable::getLabel());
