...
AllocaInitInstruction::AllocaInitInstruction(Operand *src, Operand *dst, SymbolEntry *se, BasicBlock *insert_bb) : Instruction(ALLOCA, insert_bb)
{
    operands.push_back(dst);
    dst->setDef(this);
    this->se = se;
...
AllocaInitInstruction::~AllocaInitInstruction()
{
    operands[0]->setDef(nullptr);
    if (operands[0]->usersNum() == 0)
        delete operands[0];
...
void AllocaInitInstruction::output() const
{
    std::string dst, type;
    dst = operands[0]->toStr();
    type = se->getType()->toStr();
...
void FunctionCallInstruction::output() const
{
    ...
    if (!type->isVoid())
        fprintf(yyout, "%s = ", operands[0]->toStr().c_str()); // 仅当返回值为非void时，向临时寄存器赋值
    fprintf(yyout, "  call %s %s(", type->toStr().c_str(), se->toStr().c_str());
