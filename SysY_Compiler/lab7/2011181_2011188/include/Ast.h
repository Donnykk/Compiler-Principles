...
public:
    ...
    static void setIRBuilder(IRBuilder *builder_) { builder = builder_; };
...
public:
    ...
    bool isInitExpr() const { return kind == INITEXPR; };
...
public:
    Id(SymbolEntry *se, ExprNode *arr_index = nullptr) : ExprNode(se), arr_index(arr_index)
    {
        if (se)
        {
            type = se->getType();
    ...
    void genCode();
    int getValue();
    ExprNode *getArrIndex() { return arr_index; };
    Type *getType();
    bool isLeft() const { return left; };
    void setLeft() { left = true; }
};

class InitExpr : public ExprNode
{
private:
    ExprNode *expr;
    int childCnt;
...
public:
    InitExpr(SymbolEntry *se, ExprNode *expr = nullptr) : ExprNode(se, INITEXPR), expr(expr)
    {
        type = se->getType();
        childCnt = 0;
    };
...
public:
    FunctionCall(SymbolEntry *se, ExprNode *param = nullptr);
    void genCode();
};
