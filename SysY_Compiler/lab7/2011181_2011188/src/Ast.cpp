...
FunctionCall::FunctionCall(SymbolEntry *se, ExprNode *param) : ExprNode(se), param(param)
{
    dst = nullptr;
    SymbolEntry *s = se;
    int num_param = 0;
...
void InitExpr::addExpr(ExprNode *expr)
{
...
void InitExpr::fill()
{
    ...
    {
        while (!isFull())
            this->addExpr(new InitExpr(new ConstantSymbolEntry(type)));
        ExprNode *temp = expr;
        while (temp)
        {
            ((InitExpr *)temp)->fill();
